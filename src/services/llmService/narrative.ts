...
export async function generateNarrativeVideoPlan(req: RenderRequest): Promise<VideoPlan> {
  ...
  console.log(`[Narrative] Duración solicitada: ${duration}, Estilo: ${style}`);
  ...
  console.log('[Narrative] Cargando assets desde assets_index.json...');
  try {
    assetsIndex = JSON.parse(await fs.readFile(path.resolve(process.cwd(), 'assets_index.json'), 'utf-8'));
  } catch (e) {
    assetsIndex = [];
  }
  if (assetsIndex.length === 0) console.error('[Narrative] No se pudieron cargar assets, el índice está vacío.');
  ...
  console.log(`[Narrative] Fondos encontrados: ${fondos.length}, Actores encontrados: ${actores.length}`);
  if (fondos.length === 0) console.warn('[Narrative] No hay fondos completos para el estilo solicitado.');
  if (actores.length === 0) console.warn('[Narrative] No hay actores completos para el estilo solicitado.');
  ...
  const systemPrompt = `Eres un generador de planes de video para producciones narrativas. Tu tarea es crear un objeto JSON llamado VideoPlan que contenga una timeline de escenas detalladas, cada una con los siguientes campos: t, visual, background, character, camera, lighting, emotion, music, fx, transition, carryover, audioCarryover, faceAnimation, lipSync. El campo visualStyle debe ser "narrative".\n\nFondos disponibles:\n${fondosList}\n\nActores disponibles:\n${actoresList}\n\nNo incluyas texto adicional, solo el JSON. Respeta la duración solicitada y no dejes segundos vacíos. Elige SIEMPRE los assets de las listas proporcionadas.`;
  const userPrompt = `Genera un VideoPlan narrative para ${duration} segundos usando únicamente los fondos y actores de las listas proporcionadas y el estilo solicitado. El output debe ser estrictamente el JSON con los campos timeline y visualStyle.`;
  const models = [
  ...
  console.warn('[Narrative] Todos los intentos LLM fallaron, usando fallback local.');
  const timeline: any[] = [];
